                ...
                {                   
                                ...
                                foreach (var i in ProcessingData)
                                {
                                    ...
                                    if (CheckMobileNo(i) == "Valid")
                                    {
                                        if (i.Create_Try < 5)
                                        {
                                            ...
                                            try
                                            {
                                                if (CreateRecharge.recharge_status == 100)
                                                {
                                                    ...
                                                    FindOrginal.Create_Status_Code =
                                                        CreateRecharge.recharge_status.ToString();
                                                    ...
                                                    db.SaveChanges();
                                                    CreateLog("Recharge Request  id: " + i.id + " Mobile No:" +
                                                              i.recepient_msisdn + " has been Processed ");
                                                    listBox1.Items.Add("Recharge Request id: " + i.id + " Mobile No:" +
                                                                       i.recepient_msisdn + " has been Processed");

                                                }
                                                else if (CreateRecharge.recharge_status == 300 ||
                                                         CreateRecharge.recharge_status == 301 ||
                                                         CreateRecharge.recharge_status == 308 ||
                                                         CreateRecharge.recharge_status == 331 ||
                                                         CreateRecharge.recharge_status == 332 ||
                                                         CreateRecharge.recharge_status == 333 ||
                                                         CreateRecharge.recharge_status == 339 ||
                                                         CreateRecharge.recharge_status == 399)
                                                {
                                                    try
                                                    {
                                                        if (i.Create_Try < 1)
                                                        {
                                                            RechargeStatusMsg = "Error" + ":" +
                                                            ...
                                                            db.SaveChanges();
                                                            CreateErrorLog("Recharge Response of id: " + i.id +
                                                                           RechargeStatusMsg + "..Tried For " +
                                                                           FindOrginal.Create_Try + " Times");
                                                        }
                                                        else
                                                        {
                                                            FindOrginal.Create_Try = FindOrginal.Create_Try + 1;
                                                ...
                                                else
                                                {
                                                    try
                                                    {
                                                        FindOrginal.Create_Status_Code =
                                                            CreateRecharge.recharge_status.ToString();
                                                        FindOrginal.create_recharge_Status =
                                                            proxy.GetStatusCodeMeaning(i.client_id,
                                                                i.Create_Status_Code);
                                                        FindOrginal.Create_Try = FindOrginal.Create_Try + 1;
                                                        FindOrginal.SentDateTime = DateTime.Now;
                                                        db.SaveChanges();
                                                        CreateErrorLog("Recharge Response of id: " + i.id +
                                                                       RechargeStatusMsg + " Tried For " +
                                                                       FindOrginal.Create_Try + " Times");
                                                        listBox1.Items.Add("Recharge Request id: " + i.id +
                                                                           "  Can't Be recharged." +
                                                                           RechargeStatusMsg);
                                                    }
                                                        #region EntityValidationErrorHandler
                                            ...
                                            {
                                                string exception = ex.Message + "," + ex.InnerException + "," +
                                                                   ex.Source;
                    ...
                    catch (Exception ex)
                    {
                        if (ex.InnerException != null)
                        {
                            ...
                            label1.Text = Msg;
                            CreateErrorLog(ex.Message + ex.InnerException + ex.Source + ex.ToString());
                        }
                        else
                        {
                            Msg = ex.Message;
